        ...
        if current_dayun:
            ...
            if highlight:
                ...
                for period, star in highlight.items():
                    ...
                    result['data']['current_dayun']['xingyao'].append({
                        'period':period,
                        'star': star,
                        'meaning':meaning,
            ...
            # 📌 使用 prints_buffer 来提前输出推荐日内容
            prints_buffer = [f"\n📅 未来{days}天择吉推荐（任务：{task}）："]

            if not good_days:
                prints_buffer.append(f"\n⚠️ 无得分 ≥ 2 的推荐日，推荐得分最高的前三日：")
            ...
            # ✅ 插入到最终输出最前面
            result['prints'] = prints_buffer + result['prints']
